...
var main = function (medicineObjects) {
	...
	$(".content").on("click", ".buttonStyle", function() {
		...
		var newMed = {"description":newDescription, "tags":newTags};

		$.post("todos", newMed, function (result) {
			organizedByTag = convertToTags2(organizeByTags(result));

			$("#description").val("");
			$("#tags").val("");
		});

		var result = updateJson(medicineObjects, newDescription, newTags);
	});
}

function updateJson(medicineObjects, newDescription, newTags) {
